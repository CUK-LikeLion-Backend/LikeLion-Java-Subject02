...
public class Student {
    public static void main(String[] args) throws IOException {
            ...
            while ((str = reader.readLine()) != null) {
                ...
                if (tmp[3].equals("전선") || tmp[3].equals("전기")) {
                    ...
                    if (tmpGrade == 0)
                        ++fCount; // F 학점인 경우
                    if (fCount >= 3)
                        throw new UnsupportFunctionException("학사경고임", 3);
                    if (tmpGrade >= 5)
                        throw new UnsupportFunctionException("학점이 옳게 부여되지 않았습니다.", 1);
                    grade += tmpGrade;
                    ++majorCount;
                }
            }
        ...
        // writer 객체는 따로 close()를 호출하지 않아도 try를 벗어나는 순간 close()가 자동으로 호출된다.
